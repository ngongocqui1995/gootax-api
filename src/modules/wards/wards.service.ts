...
@Injectable()
export class WardsService extends TypeOrmCrudService<Ward> {
  model_name: string = ENUM_MODEL.WARD;
  status_name: string = ENUM_MODEL.STATUS;

  constructor(
    @InjectRepository(Ward) repo,
  ...
  async replaceOneBase(
    ...
    const [err] = await to(this.replaceOne(req, <Ward>dto));
  ...
  async updateOneBase(
    ...
    const [err] = await to(this.updateOne(req, <Ward>dto));
    if (err) this.checkService.throwErrorSystem(err.message);
    return {
      status: HttpStatus.OK,
      message: await this.checkService.i18n.translate(
  ...
  async createOneBase(
    ...
    const [err] = await to(this.createOne(req, <Ward>dto));
    if (err) this.checkService.throwErrorSystem(err.message);
